...
namespace FavoriteCims
{
	public class VechiclePassengersButton : UIButton
	{
		...
		FavCimsVechiclePanel VehiclePanel;
		...
		public override void Start() {
			...
			if (FavCimsMainClass.FullScreenContainer.GetComponentInChildren<FavCimsVechiclePanel> () != null) {
		...
		public override void Update() {
			...
			if (this.isVisible) {
				...
				if(CitizenVehicleWorldInfoPanel.GetCurrentInstanceID() != InstanceID.Empty) {
					VehicleID = CitizenVehicleWorldInfoPanel.GetCurrentInstanceID();
				}
	...
	public class FavCimsVechiclePanel : UIPanel
	{
		...
		UIPanel DriverPanel;
		UIPanel DriverPanelSubRow;
		UIButton DriverPanelIcon;
		UIButton DriverPanelText;
		DriverPrivateVehiclePanelRow DriverPrivateBodyRow;

		UIPanel PassengersPanel;
		UIPanel PassengersPanelSubRow;
		UIButton PassengersPanelIcon;
		UIButton PassengersPanelText;
		PassengersPrivateVehiclePanelRow[] PassengersPrivateBodyRow = new PassengersPrivateVehiclePanelRow[5];
		...
		public override void Start() {
			try
			{
				this.width = 250;
				this.height = 0;
				this.name = "FavCimsVechiclePanel";
				//this.backgroundSprite = "PoliciesBubble";
				...
				DriverPanel.width = 226;
				DriverPanel.height = 25;
				...
				DriverPanelSubRow.width = 226;
				DriverPanelSubRow.height = 25;
				...
				DriverPanelText.textScale = 0.80f;
				DriverPanelText.useDropShadow = true;
				DriverPanelText.dropShadowOffset = new Vector2 (1, -1);
				DriverPanelText.dropShadowColor = new Color32 (0, 0, 0, 0);
				DriverPanelText.textPadding.left = 5;
				DriverPanelText.textPadding.right = 5;
				DriverPanelText.textColor = new Color32 (51, 51, 51, 160); //r,g,b,a
				DriverPanelText.isInteractive = false;
				DriverPanelText.relativePosition = new Vector3 (DriverPanelIcon.relativePosition.x + DriverPanelIcon.width, 1);
				...
				DriverPrivateBodyRow.citizen = 0;
				...
				for(int a = 1; a < 5; a++) {
					PassengersPrivateBodyRow[a] = BodyRows.AddUIComponent (typeof(PassengersPrivateVehiclePanelRow)) as PassengersPrivateVehiclePanelRow;
					PassengersPrivateBodyRow[a].name = "RowPanel_Passengers_" + a.ToString();
					PassengersPrivateBodyRow[a].OnVehicle = 0;
					PassengersPrivateBodyRow[a].citizen = 0;
					PassengersPrivateBodyRow[a].Hide();
				}
		...
		public override void Update() {
				...
				if(Garbage) {
					...
					try
					{
						...
						for(int a = 1; a < 5; a++) {
							...
							PassengersPrivateBodyRow[a].citizen = 0;
							PassengersPrivateBodyRow[a].OnVehicle = 0;
							PassengersPrivateBodyRow[a].firstRun = true;
						}
			...
			try
			{
				if(!CitizenVehicleWorldInfoPanel.GetCurrentInstanceID().IsEmpty && CitizenVehicleWorldInfoPanel.GetCurrentInstanceID() != VehicleID) {
				...
				if (this.isVisible && !VehicleID.IsEmpty) {
					...
					if(50 + ((float)CimsOnVeh.Count * 25) < (this.height - Title.height - Footer.height)) {
						Body.height = this.height - Title.height - Footer.height;
					} else if(50 + ((float)CimsOnVeh.Count * 25) > 400) {
						Body.height = 400;
					} else {
						Body.height = 50 + ((float)CimsOnVeh.Count * 25);
					}
					...
					if(execute) {
						...
						while (VehicleUnits != 0) {
							...
							for (int i = 0; i < 5; i++) {
								...
								if (citizen != 0 && !CimsOnVeh.ContainsKey(citizen)) {
									...
									if(i == 0) {
										...
										if(PassengersPanel != null && PassengersPrivateBodyRow[i] != null) {	

											CimsOnVeh.Add(citizen, VehicleUnits);

											PassengersPrivateBodyRow[i].citizen = citizen;
											PassengersPrivateBodyRow[i].OnVehicle = VehicleID.Vehicle;
											PassengersPrivateBodyRow[i].firstRun = true;
											PassengersPrivateBodyRow[i].Show();
										}
									}
								}
							}
